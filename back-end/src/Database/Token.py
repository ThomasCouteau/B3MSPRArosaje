...
class TokenTable:
    ...
    def Generate(self, user: User) -> Token:
        ...
        if user.token is not None:                                                                                      # Si l'utilisateur a déjà un token
            self.db.table('token').update({"accessToken": accessToken, "refreshToken": refreshToken, "expire": (datetime.datetime.now() + datetime.timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S.%f")}).eq('userID', user.id).execute()
        else:                                                                                                           # Si l'utilisateur n'a pas de token
            self.db.table('token').insert({"userID": user.id, "accessToken": accessToken, "refreshToken": refreshToken, "expire": (datetime.datetime.now() + datetime.timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S.%f")}).execute()
    ...
    def Refresh(self, token: Token) -> Token:
        ...
        accessToken = ''.join(random.choices("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", k=64))
        refreshToken = ''.join(random.choices("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", k=64))
        self.db.table('token').update({"accessToken": accessToken, "refreshToken": refreshToken, "expire": (datetime.datetime.now() + datetime.timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S.%f")}).eq('refreshToken', token.refreshToken).execute()
        return self.GetByRefreshToken(refreshToken)
